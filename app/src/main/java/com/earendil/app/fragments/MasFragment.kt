...
class MasFragment : Fragment(R.layout.activity_mas_fragment) {
    ...
    private fun reproducirAudio() {
        try {
            ...
            // Configuramos el MediaPlayer en lugar de Ringtone
            val mediaPlayer = MediaPlayer.create(requireContext(), sonido)
            mediaPlayer.setOnCompletionListener {
                it.release() // Liberamos memoria al terminar
            }
            mediaPlayer.start()

            tvResultado.text = "Recurso multimedia de audio reproducido."
        } catch (e: Exception) {
            tvResultado.text = "Error al reproducir el audio."
            e.printStackTrace()
        }
    }
